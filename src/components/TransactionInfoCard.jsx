...
const TransactionInfoCard = ({icon, title, date , amount, type, hideDeleteBtn, onDelete}) => {
  ...
  return (
    <div className="group relative flex items-center gap-4 mt-2 p-3 rounded-lg hover:bg-gray-100/60">
        <div className="w-12 h-12 flex items-center justify-center text-xl textgray-800 bg-gray-100 rounded-full">
            {icon ? (
        ...
        <div className="flex-1 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
                {!hideDeleteBtn && (
                    <button
                    onClick={onDelete} 
                    className="text-gray-400 hover:text-red-600 opacity-60 group hover:opacity-100 transition-opacity cursor-pointer">
                        <Trash2 size={18}/>    
                    </button>
                )}
